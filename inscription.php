...
if(isset($_POST['submit']))
{
    if(!empty($_POST['prenom']) AND!empty($_POST['nom']) AND!empty($_POST['login']) AND!empty($_POST['password'])) // Avec else, il va afficher un message si des champs ont étés oubliés.
    {
        ...
        $insertData = $bdd->prepare('INSERT INTO `utilisateurs`(`login`, `prenom`, `nom`, `password`) VALUES (?, ?, ?, ?)');
        $insertData->execute(array($login, $prenom, $nom, $password));
        $grabData = $bdd->prepare('SELECT * FROM `utilisateurs` WHERE `login` = ? AND `prenom` = ? AND `nom`= ? AND `password` = ?');
        $grabData->execute(array($login, $prenom, $nom, $password));
        ...
        echo "Merci de ton inscription " . $_POST['login'] . " et bienvenue dans mon SQL."; // Message que l'inscription a bien été prise en compte.
    }
    else
    {
